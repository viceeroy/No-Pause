...
| `src/services/aiFeedback.ts` | Groq-backed AI feedback helpers, transcript usability helper, practice feedback generation | `api/feedback.ts`, `src/lib/telegram/voiceHandler.ts` |
| `src/services/deepgram.ts` | Deepgram nova-3 transcription types and server-side transcription helper with word timestamps | `api/transcription.ts`, `src/lib/telegram/voiceHandler.ts` |
| `src/services/groq.ts` | Groq chat completions text generation client | `src/services/aiFeedback.ts` |
