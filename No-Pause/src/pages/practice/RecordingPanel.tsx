...
export function RecordingPanel({
  ...
  return (
    <div className="flex-1 flex flex-col items-center max-w-2xl mx-auto animate-in fade-in duration-700 overflow-hidden pb-20">
      <div className="w-full shrink-0">
        {mode === 'lemon' && (
          <div className="p-6 md:p-10 bg-ember-200/10 border-2 border-ember-500/35 rounded-[32px] shadow-card relative overflow-hidden">
            <div className="absolute top-0 right-0 p-4 md:p-6">
              <div className="flex items-center gap-2 px-4 py-1.5 bg-ember-500 text-slate-950 rounded-full text-sm font-bold font-sans shadow-sm">
                <Timer size={16} className="animate-pulse" /> {formatTime(timeLeft)}
              </div>
            </div>
            <p className="text-[10px] text-ember-600 uppercase tracking-widest font-black mb-3">Topic focus:</p>
            <div className="text-4xl md:text-5xl font-serif font-bold text-foreground">{lemonPrompt?.word}</div>
          </div>
        )}
        {mode === 'topic' && topicPrompt && (
          <div className="p-10 bg-cyan-500/10 border-2 border-cyan-400/35 rounded-[40px] shadow-card relative overflow-hidden">
            ...
            <div className="text-xl md:text-2xl font-serif font-medium text-foreground leading-snug">{topicPrompt.topicTitle}</div>
          </div>
        )}
        {mode === 'free' && (
      ...
      <div className="shrink-0 pt-4">
        <button onClick={() => void stopRecording()} className="w-full md:w-auto px-16 py-4 rounded-full bg-primary hover:brightness-110 text-primary-foreground font-sans font-black text-lg btn-press shadow-soft night-glow flex items-center justify-center gap-4">
