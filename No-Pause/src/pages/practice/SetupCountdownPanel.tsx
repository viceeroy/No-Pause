...
export function SetupCountdownPanel({
  ...
  return (
    <div className="text-center flex-1 flex flex-col justify-start overflow-y-auto pb-28">
      {transcriptError && (
      ...
      <div className={cn('transition-all duration-500 flex-1 flex flex-col justify-start md:justify-center min-h-0', state === 'countdown' && 'opacity-30 scale-95 blur-[2px]')}>
        {mode === 'lemon' && (
          <div className="p-6 md:p-10 bg-ember-200/10 border border-ember-500/35 rounded-[32px] shadow-card">
            <p className="text-xs text-ember-600 uppercase tracking-widest font-bold mb-3">You will speak about:</p>
            <div className="text-4xl md:text-6xl font-serif font-bold text-foreground mb-4">{lemonPrompt?.word || 'Loading...'}</div>
            ...
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-ember-300/25 text-ember-600 rounded-full text-sm font-sans font-bold border border-ember-500/30 mb-4">
        ...
        {mode === 'topic' && topicPrompt && (
          <div className="p-6 md:p-10 bg-cyan-500/10 border border-cyan-400/35 rounded-[32px] shadow-card">
            <p className="text-xs text-cyan-300 uppercase tracking-widest font-bold mb-3">You will speak about:</p>
            <div className="text-xl md:text-2xl font-serif font-medium text-foreground leading-snug mb-4">{topicPrompt.topicTitle}</div>
            ...
            <div className="mt-5 text-left max-w-xl mx-auto">
              <p className="text-xs text-cyan-300 uppercase tracking-widest font-bold mb-2">You should say:</p>
              <ul className="space-y-1.5">
                {topicPrompt.cueCard.map((step) => (
                  <li key={step} className="text-sm text-muted-foreground font-sans">• {step}</li>
                ))}
              </ul>
            </div>
            <div className="mt-5">
              <p className="text-xs text-cyan-300 uppercase tracking-widest font-bold mb-2">Difficulty Mode</p>
          ...
          <div className="flex flex-col md:flex-row items-center justify-center gap-3 w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
            {(mode === 'lemon' || mode === 'topic') && (
              <button
                ...
                className="w-full md:w-auto px-8 py-4 rounded-full bg-surface-card border border-border hover:bg-surface-elevated disabled:opacity-50 disabled:cursor-not-allowed text-foreground font-sans font-bold btn-press flex items-center justify-center gap-2 shadow-card"
              >
            ...
            <button
              ...
              className="w-full md:w-auto px-10 py-4 rounded-full bg-primary hover:brightness-110 disabled:opacity-50 disabled:cursor-not-allowed text-primary-foreground font-sans font-bold btn-press flex items-center justify-center gap-2 shadow-soft night-glow"
            >
