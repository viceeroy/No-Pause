...
export function SetupCountdownPanel({
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (['INPUT', 'TEXTAREA', 'BUTTON'].includes(targetTag)) return;
  ...
  }, [state, handleStartClick, slides.length]);
        ...
        {state === 'setup' ? (
          <div className="flex min-h-[calc(100dvh-230px)] flex-col items-center justify-between py-1 md:min-h-0 md:justify-start md:gap-8 md:py-0">
            <div
              ...
              tabIndex={0}
              className={cn(
                'w-full overflow-hidden select-none rounded-[28px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2',
            ...
            <div className="flex w-full flex-col items-center gap-4 pb-2 pt-6 md:pt-0 md:pb-0">
              ...
              <button
                ...
                className="flex w-full items-center justify-center gap-2.5 rounded-full bg-primary px-10 py-4 text-base font-sans font-black text-primary-foreground shadow-soft btn-press hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50 md:w-auto sm:px-16 sm:text-lg"
              >
                <span>Start Speaking</span>
                <span className="hidden opacity-70 md:inline font-sans font-normal text-sm">[Space]</span>
              </button>
            </div>
          </div>
        ) : (
