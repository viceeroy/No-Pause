-- Scope all-time totals to completed sessions only.
-- `totalSessions` and `totalPracticeTime` in get_practice_stats previously
-- counted every row (including sub-5s rows with completed=false). The Stats
-- page Sessions / Practice time cards should reflect completed sessions only.
-- Weekly/avg/best/recent paths unchanged.
