...
__export(utils_exports, {
  ...
  selectRegionOrUseDefault: () => selectRegionOrUseDefault,
  sorted: () => sorted,
...
async function processDeployKeyValue(ctx, deployKey) {
  if (deployKey === void 0) {
    return void 0;
  }
...
Authenticate with \`npx convex dev\``;
    ...
    return await ctx.crash({
      ...
      errForSentry: this,
      printedMessage: import_chalk.chalkStderr.red(msg.trim())
    });
  }
}
...
async function logAndHandleFetchError(ctx, err) {
  ...
  if (err instanceof ThrowingFetchError) {
    ...
    return await ctx.crash({
      ...
      errorType: "transient",
      errForSentry: err,
      printedMessage: import_chalk.chalkStderr.red(err)
    });
  }
}
function logDeprecationWarning(ctx, deprecationMessage) {
  ...
  (0, import_log.logWarning)(import_chalk.chalkStderr.yellow(deprecationMessage));
}
    ...
    switch (deprecationState) {
      ...
      case "Deprecated":
        return await ctx.crash({
          exitCode: 1,
          errorType: "fatal",
          printedMessage: import_chalk.chalkStderr.red(deprecationMessage)
        });
      default:
...
async function validateOrSelectTeam(ctx, teamSlug, promptMessage) {
  ...
  if (teams.length === 0) {
    await ctx.crash({
      ...
      printedMessage: import_chalk.chalkStderr.red("Error: No teams found")
    });
  }
  if (!teamSlug) {
    switch (teams.length) {
      ...
      default: {
        const teamSlug2 = await (0, import_prompts.promptSearch)(ctx, {
          message: promptMessage,
          choices: teams.map((team2) => ({
            name: `${team2.name} (${team2.slug})`,
            value: team2.slug
          }))
        });
        const team = teams.find((team2) => team2.slug === teamSlug2);
        if (!team) {
          return await ctx.crash({
            exitCode: 1,
            errorType: "fatal",
    ...
    const team = teams.find((team2) => team2.slug === teamSlug);
    if (!team) {
      return await ctx.crash({
        exitCode: 1,
        errorType: "fatal",
...
async function selectRegionOrUseDefault(ctx, selectedTeam) {
  const noDefaultRegionMessage = `Configure a default region for your team at ${import_chalk.chalkStderr.bold(`https://dashboard.convex.dev/t/${selectedTeam.slug}/settings`)}`;
  if (!process.stdin.isTTY) {
    if (!selectedTeam.defaultRegion) {
      (0, import_log.logMessage)(noDefaultRegionMessage);
    }
    return selectedTeam.defaultRegion ?? null;
  }
  const selectedRegionName = selectedTeam.defaultRegion ?? await selectRegion(ctx, selectedTeam.id);
  if (!selectedTeam.defaultRegion) {
    (0, import_log.logMessage)(noDefaultRegionMessage);
  }
  return selectedRegionName;
}
async function selectRegion(ctx, teamId) {
  ...
  const choices = regionsResponse.items.filter((item) => Boolean(item.available)).map((item) => ({
...
const typedBigBrainClient = typedBigBrainClientFactory(BIG_BRAIN_URL);
const typedPlatformClient = typedBigBrainClientFactory(
