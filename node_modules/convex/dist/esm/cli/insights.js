...
import { fetchInsights } from "./lib/insights.js";
...
function formatInsight(insight, details) {
  const severity = insight.severity === "error" ? chalk.red(`[ERROR]`) : chalk.yellow(`[WARNING]`);
  const kind = formatInsightKind(insight.kind);
  const fn = chalk.bold(formatFunctionName(insight));
  let detail;
  if ("occCalls" in insight) {
    const table = insight.occTableName ? ` on table ${chalk.cyan(insight.occTableName)}` : "";
...
export const insights = new Command("insights").summary("Show health insights for your deployment").description(
...
).allowExcessArguments(false).option("--details", "Show recent events for each insight", false).addDeploymentSelectionOptions(actionDescription("Show insights for")).showHelpAfterError().action(async (cmdOptions) => {
