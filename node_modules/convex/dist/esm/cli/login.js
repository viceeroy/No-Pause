...
async function handleLinkingDeployments(ctx, args) {
  ...
  if (projectLocal !== null && isAnonymousDeployment(projectLocal.deploymentName)) {
...
async function linkSingleDeployment(ctx, deploymentName, configuredDeployment, options) {
  const teamSlug = options?.teamSlug ?? (await validateOrSelectTeam(
    ctx,
    void 0,
    "Choose a team for your deployment:"
  )).team.slug;
  const projectSlug = options?.projectSlug ?? (await selectProject(ctx, "ask", {
    team: teamSlug,
    devDeployment: "local",
